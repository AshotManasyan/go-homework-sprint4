...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	averegRun := meanSpeed(action, duration)
	calRun := ((runningCaloriesMeanSpeedMultiplier * averegRun * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return calRun
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	averegWalk := meanSpeed(action, duration) * kmhInMsec
	calWalk := ((walkingCaloriesWeightMultiplier * weight + (math.Pow(averegWalk, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
	return calWalk
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	averegSwim := swimmingMeanSpeed(lengthPool, countPool, duration)
	calSwim := (averegSwim + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
	return calSwim
}
